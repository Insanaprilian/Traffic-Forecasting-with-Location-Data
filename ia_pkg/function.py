...
def data_split(df,
    ...
    if len(sizes) != len(names) or sum(sizes) != 1:
        ...
        if len(names) == 2:
            train = int(sizes[0] * l)
            dtype = np.where(df.index.isin(p[:train]), names[0], names[1])  
        if len(names) > 2:
            train = int(sizes[0] * l)
            valid = int(sizes[1] * l) + train
            dtype = np.where(df.index.isin(p[:train]), names[0],
                        np.where(df.index.isin(p[train:valid]), names[1], names[2]))  
