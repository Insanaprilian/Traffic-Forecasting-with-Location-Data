...
def dist_plot(df,columns,col_target):
    ...
    for c, i in zip(columns,range(0,r)):
        ...
        lgd1 = ax2.legend(bbox_to_anchor=(1, 0.8), loc='upper right', borderaxespad=0.1)
        _ = plt.title(c)
        _ = plt.show()
...
def plot_score_linearity(df, col_target, col_score, bins=10, ax=None):
    ...
    df['bin'], labels = pd.qcut(df[col_score], bins, retbins=True, precision=5, duplicates='drop')
    # labels
